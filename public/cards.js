...
const adminRoute = "https://backend-for-sgdms-1.onrender.com/adminRoute";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector(".logout").addEventListener("click", () => {
    fetch("https://backend-for-sgdms-1.onrender.com/dashAdmin/logout", {
      method: "POST",
    })
      .then((res) => {
        return res.json();
  ...
  fetch(getBins)
    ...
    .then((data) => {
      ...
      let driverNames = [name5, name4, name3, name2, name1];
      ...
      //backend-for-sgdms-1.onrender.com/totalBins
      fetch("http://localhost:3000/totalBins", {
        method: "POST",
        headers: {
      ...
      for (let i = 0; i < noOfDrivers; i++) {
        if (data.drivers[i].id == data.bins[0].myBins[i].id) {
          driverNames[i].innerHTML = data.drivers[i].driverName;
        }
      }

      // Update chart6 with new data
      chart6.updateSeries([
        {
      ...
      loc5.innerHTML = binLocations[0];
      loc4.innerHTML = binLocations[1];
      loc3.innerHTML = binLocations[2];
      loc2.innerHTML = binLocations[3];
      loc1.innerHTML = binLocations[4];
    })
    .catch((err) => {
